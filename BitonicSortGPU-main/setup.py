...
def compile_cuda():
    ...
    result = subprocess.run(dlink_cmd)
    if result.returncode != 0:
        raise RuntimeError("Failed to device‐link CUDA source")

    return ["bitonic.o", dlink_obj]


class CustomBuildExt(build_ext):
    def build_extensions(self):
        # Compile CUDA source first
        cuda_objs = compile_cuda()

        # Add CUDA object file to each extension
        for ext in self.extensions:
            ext.extra_objects.extend(cuda_objs)
